<script setup lang="ts">
  ...
  async function signIn() {
    try {
      !isUserAlreadySignup.value
        ? await login(ruleForm.email, ruleForm.password)
        : await signup(ruleForm.email, ruleForm.password)
    } catch (ex: any) {
    } finally {
      emit('toggleSession')
    }
  ...
  <div>
    ...
    <div v-if="!isLoggedInUser">
      <div class="login-intro">
        <h2>
          AITexter is <span style="color: #a745ce">Much Much</span> better for
          Users!
        </h2>
        <p style="text-align: center">
          <span>{{ !isUserAlreadySignup ? 'Login' : 'Signup' }}</span> to
          discover our
          <span style="font-weight: 1000; text-decoration: underline"
            >Extra</span
          >
          free features.
        </p>
      ...
      <div class="login-form-container">
        ...
        <h5
          @click="() => toggleLoginSignupState()"
          v-if="!isUserAlreadySignup.valueOf()"
          style="text-align: center">
          Not a user yet ? click
          <span style="cursor: pointer; color: #a855c8">HERE</span> to sign up
        </h5>
        <h5
          @click="() => toggleLoginSignupState()"
          v-else
          style="text-align: center">
          Needs to log in ? click
          <span style="color: #a855c8; cursor: pointer">HERE</span>
        </h5>
      </div>
    </div>
  </div>
